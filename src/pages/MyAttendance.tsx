...
import { Clock, Calendar, MapPin, AlertCircle, RefreshCw, Settings } from 'lucide-react';
...
const MyAttendance = () => {
  ...
  // Check if user can manage attendance (Senior Partner or Superadmin)
  const isSeniorPartner = employeePosition?.toLowerCase() === 'senior partner';
  const canManageAttendance = userrole === 'superadmin' || isSeniorPartner;
    ...
    <ResponsiveLayout>
      <div className={`space-y-6 ${isMobile ? 'space-y-4' : ''}`}>
        ...
        <Tabs defaultValue="my-attendance" className="w-full">
          <TabsList className={`grid w-full ${canManageAttendance ? 'grid-cols-2' : 'grid-cols-1'}`}>
              ...
              <Card>
                ...
                <CardContent>
                  <p className={`font-bold text-blue-600 ${isMobile ? 'text-xl' : 'text-2xl'}`}>
                    {presentDays > 0 ? `${Math.round((presentDays / 22) * 100)}%` : '0%'}
                  </p>
                </CardContent>
              </Card>
            </div>

            {/* Month Filter and Attendance Records */}
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
