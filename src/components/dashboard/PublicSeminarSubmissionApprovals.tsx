...
import { CheckCircle, XCircle, UserSearch, GraduationCap, Pencil, UserPlus, RefreshCw, ListFilter, ArrowUpDown } from 'lucide-react';
...
const PublicSeminarSubmissionApprovals: React.FC<Props> = ({ branchId }) => {
  ...
  React.useEffect(() => {
    ...
    const auto = pickAutoMatch(matches as SeminarStudentMatch[], { subject: subjectOf(matchingSub) });
    if (!auto) return;
  ...
  const runScan = React.useCallback(async (rows: PendingSeminarSubmission[]) => {
    ...
    try {
      const matchRes = await runAutoMatchSweep('seminar-submissions', rows, {
        ...
        matchStudent: (s, sid) => matchSeminarSubmission(s.id, sid),
    ...
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="flex items-center gap-2 text-base sm:text-lg">
          ...
          <MatchHistoryDialog scope='seminar' />
        </CardTitle>
      </CardHeader>
