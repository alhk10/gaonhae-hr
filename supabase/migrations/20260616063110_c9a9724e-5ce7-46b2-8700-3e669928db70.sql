...
ALTER TABLE public.competition_events
  ADD COLUMN IF NOT EXISTS indemnity_template_url text NULL,
  ADD COLUMN IF NOT EXISTS indemnity_template_name text NULL;

-- Drop old signatures so we can change them
DROP FUNCTION IF EXISTS public.admin_upsert_competition_event(uuid, text, boolean, integer, text, boolean, boolean, boolean, text, numeric, jsonb, boolean);
...
RETURNS TABLE(
  ...
  indemnity_clause text, require_indemnity_form boolean, require_passport boolean, require_photo boolean,
  coaching_label text, coaching_amount numeric, extra_lines jsonb, coaching_required boolean,
