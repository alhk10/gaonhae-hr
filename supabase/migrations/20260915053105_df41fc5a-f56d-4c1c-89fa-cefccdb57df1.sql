...
BEGIN
  ...
  -- Use available student credit first
...
BEGIN
  ...
  IF v_invoice_id IS NOT NULL THEN
    ...
    PERFORM public.consume_credit_hold(v_invoice_id, p_verified_by);
...
BEGIN
  ...
  IF v_invoice_id IS NOT NULL THEN
    ...
    PERFORM public.release_credit_hold(v_invoice_id, p_reviewed_by);
...
CREATE OR REPLACE FUNCTION public.admin_delete_school_fees_submission(p_id uuid, p_deleted_by text DEFAULT 'admin'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  IF v_invoice_id IS NOT NULL THEN
    PERFORM public.release_credit_hold(v_invoice_id, p_deleted_by);
