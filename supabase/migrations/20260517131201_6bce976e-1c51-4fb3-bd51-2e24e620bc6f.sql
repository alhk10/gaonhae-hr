CREATE OR REPLACE FUNCTION public.get_public_grading_slots(p_branch_id text, p_product_ids uuid[], p_dob date DEFAULT NULL)
 RETURNS TABLE(id uuid, branch_id text, branch_name text, branch_address text, grading_date date, start_time time without time zone, end_time time without time zone, location text)
 LANGUAGE sql
