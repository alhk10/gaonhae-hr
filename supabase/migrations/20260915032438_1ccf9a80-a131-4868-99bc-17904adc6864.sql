...
BEGIN
  ...
  SELECT s.current_belt,
         CASE WHEN s.date_of_birth IS NULL THEN NULL
              ELSE date_part('year', age(current_date, s.date_of_birth))::int END
    INTO v_current_belt, v_age
    ...
    WHERE COALESCE(p.is_active, true) = true
      ...
      AND (
        (p_category_id = v_school_fee_category AND (
          ...
          OR (
            p.name NOT IN ('Trial Lesson', 'Private Lesson')
            AND EXISTS (
              SELECT 1
            ...
            AND (p.min_age IS NULL OR v_age IS NULL OR v_age >= p.min_age)
            AND (p.max_age IS NULL OR v_age IS NULL OR v_age <= p.max_age)
          )
        ))
