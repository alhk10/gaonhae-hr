...
CREATE OR REPLACE FUNCTION public.admin_update_grading_remark(
  ...
  UPDATE public.grading_registrations SET remark = NULLIF(btrim(p_remark), '') WHERE id = p_registration_id;
$$;

CREATE OR REPLACE FUNCTION public.admin_update_grading_submission_remark(
  p_submission_id uuid, p_remark text
) RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path=public AS $$
  UPDATE public.grading_payment_submissions SET remark = NULLIF(btrim(p_remark), '') WHERE id = p_submission_id;
$$;
