ALTER TABLE public.seminar_events
  ADD COLUMN IF NOT EXISTS branch_ids text[] NOT NULL DEFAULT '{}'::text[],
  ADD COLUMN IF NOT EXISTS belts text[] NOT NULL DEFAULT '{}'::text[];

DROP FUNCTION IF EXISTS public.get_public_seminar_events();

CREATE OR REPLACE FUNCTION public.get_public_seminar_events()
 RETURNS TABLE(id uuid, name text, is_active boolean, display_order integer, packages jsonb, indemnity_clause text, indemnity_template_url text, indemnity_template_name text, require_passport boolean, require_photo boolean, require_grading_card boolean, multi_package_discount boolean, branch_ids text[], belts text[])
 LANGUAGE sql
...
DROP FUNCTION IF EXISTS public.admin_upsert_seminar_event(uuid, text, boolean, integer, jsonb, text, text, text, boolean, boolean, boolean);
DROP FUNCTION IF EXISTS public.admin_upsert_seminar_event(uuid, text, boolean, integer, jsonb, text, text, text, boolean, boolean, boolean, boolean);
