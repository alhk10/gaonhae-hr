...
CREATE OR REPLACE FUNCTION public.admin_update_grading_result(
...
AS $function$
BEGIN
  IF p_result IS NOT NULL AND p_result NOT IN ('pass','fail','double','confirmed') THEN
    RAISE EXCEPTION 'Invalid result value: %', p_result;
  END IF;
  UPDATE public.grading_registrations
    SET result = p_result,
        result_manual_override = true
    WHERE id = p_registration_id;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.admin_update_grading_result(uuid, text) TO anon, authenticated;