...
DROP FUNCTION IF EXISTS public.admin_update_grading_registration_branch(uuid, text);

CREATE OR REPLACE FUNCTION public.admin_update_grading_registration_branch(p_registration_id uuid, p_branch_id text)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET branch_id = p_branch_id,
      grading_slot_id = COALESCE(v_new_slot, grading_slot_id)
  WHERE id = p_registration_id;

  RETURN v_new_slot IS NOT NULL;
END;
$function$;
