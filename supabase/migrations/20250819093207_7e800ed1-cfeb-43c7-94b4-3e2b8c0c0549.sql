...
-- Allow all authenticated users to read superladmin status (needed for role checking)
CREATE POLICY "Allow reading superladmin status" 
ON public.superadmin_users 
FOR SELECT 
TO authenticated 
USING (true);

-- Only allow superladmins to insert/update/delete (using auth.email() directly)
CREATE POLICY "Superladmins can manage users" 
ON public.superladmin_users 
FOR ALL 
TO authenticated 
...
CREATE POLICY "System can insert superladmin users" 
ON public.superadmin_users 
FOR INSERT 
TO service_role 
WITH CHECK (true);