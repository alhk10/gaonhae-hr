ALTER TABLE public.seminar_events
  ADD COLUMN IF NOT EXISTS min_packages integer NOT NULL DEFAULT 2;

ALTER TABLE public.seminar_events
  DROP CONSTRAINT IF EXISTS seminar_events_min_packages_check;
ALTER TABLE public.seminar_events
  ADD CONSTRAINT seminar_events_min_packages_check CHECK (min_packages >= 2);
...
CREATE OR REPLACE FUNCTION public.admin_upsert_seminar_event(
  ...
  p_min_packages integer DEFAULT 2
)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  IF p_id IS NULL THEN
    INSERT INTO public.seminar_events (
      ...
      GREATEST(COALESCE(p_min_packages, 2), 2)
  ...
  ELSE
    UPDATE public.seminar_events SET
      ...
      min_packages = GREATEST(COALESCE(p_min_packages, min_packages), 2),
      updated_at = now()
    WHERE id = p_id
    RETURNING id INTO v_id;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.admin_upsert_seminar_event(uuid, text, boolean, integer, jsonb, text, text, text, boolean, boolean, boolean, boolean, text[], text[], integer) TO anon, authenticated, service_role;