...
-- 2. Updated get_public_grading_list
...
-- 3. Admin RPCs
CREATE OR REPLACE FUNCTION public.admin_update_grading_registration_slot(
...
AS $function$
  ...
  SET grading_slot_id = p_slot_id
  WHERE id = p_registration_id;
$function$;

GRANT EXECUTE ON FUNCTION public.admin_update_grading_registration_slot(uuid, uuid) TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.admin_update_grading_registration_branch(
  p_registration_id uuid,
  p_branch_id text
)
RETURNS void
...
DECLARE
  ...
  v_new_slot uuid;
BEGIN
  SELECT gr.grading_slot_id, gs.grading_date
    INTO v_current_slot, v_current_date
  FROM public.grading_registrations gr
  LEFT JOIN public.grading_slots gs ON gs.id = gr.grading_slot_id
  WHERE gr.id = p_registration_id;

  -- Try to find an equivalent slot in the target branch on the same date
  ...
  UPDATE public.grading_registrations
  SET grading_slot_id = COALESCE(v_new_slot, grading_slot_id)
  WHERE id = p_registration_id;
END;
$function$;

GRANT EXECUTE ON FUNCTION public.admin_update_grading_registration_branch(uuid, text) TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.admin_update_grading_registration_display_name(
  p_registration_id uuid,
  p_display_name text
)
RETURNS void
...
GRANT EXECUTE ON FUNCTION public.admin_update_grading_registration_display_name(uuid, text) TO anon, authenticated;

CREATE OR REPLACE FUNCTION public.admin_update_student_certificate_name(
  p_student_id uuid,
...
AS $function$
  ...
  SET certificate_name = NULLIF(btrim(p_certificate_name), ''),
      updated_at = now()
  WHERE id = p_student_id;
$function$;

GRANT EXECUTE ON FUNCTION public.admin_update_student_certificate_name(uuid, text) TO anon, authenticated;
