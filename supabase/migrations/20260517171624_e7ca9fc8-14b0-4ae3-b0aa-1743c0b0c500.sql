CREATE OR REPLACE FUNCTION public.get_public_grading_slots(
  ...
  p_current_belt text DEFAULT NULL::text
)
RETURNS TABLE(
  id uuid,
  branch_id text,
...
AS $function$
  ...
  WHERE gs.grading_date >= CURRENT_DATE
    ...
    -- Belt gating: if slot specifies belt_levels and caller passes a belt, require match.
    -- Empty/NULL belt_levels means slot is open to any belt.
    AND (
    ...
    -- Product gating: only apply when caller provides product ids AND slot has restrictions.
    AND (
