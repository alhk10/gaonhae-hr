...
AS $function$
  SELECT
    gs.id,
    COALESCE(gs.branch_id, p_branch_id) AS branch_id,
    COALESCE(b.name, bp.name) AS branch_name,
    COALESCE(b.address, bp.address) AS branch_address,
    gs.grading_date,
    gs.start_time,
    gs.end_time,
    gs.location,
  ...
  LEFT JOIN public.branches bp ON bp.id = p_branch_id
  LEFT JOIN LATERAL (
