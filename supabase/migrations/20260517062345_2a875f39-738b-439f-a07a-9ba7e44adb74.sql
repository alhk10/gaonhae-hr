...
RETURNS TABLE (id text, name text, country text)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  WHERE b.name NOT IN ('Competition','Headquarters')
  ORDER BY b.name;
$$;

GRANT EXECUTE ON FUNCTION public.get_public_branches() TO anon, authenticated;

-- Public payment options: returns paynow QR, bank info, resolved grading product, and next slot
...
BEGIN
  ...
  -- Pick the active template for this branch, fallback to country-level template
  ...
  -- Resolve grading product: category=grading, name starts with current belt "<belt> >>"
  ...
  -- Resolve next upcoming grading slot for this branch + belt
