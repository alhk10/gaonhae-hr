DROP FUNCTION IF EXISTS public.get_public_chat_products_for_student(uuid, uuid, text, uuid);
...
BEGIN
  ...
  SELECT
    ...
    (p.category_id = v_school_fee_category) AS is_term_based
...
GRANT EXECUTE ON FUNCTION public.get_public_chat_terms_for_student(uuid, uuid, text) TO anon, authenticated;
