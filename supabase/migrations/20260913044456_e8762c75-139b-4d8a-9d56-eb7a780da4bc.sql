...
AS $function$
  SELECT
    ...
    gr.id,
    gs.id,
    COALESCE(s.branch_id, gs.branch_id),
    COALESCE(bs.name, b.name),
    COALESCE(bs.country, b.country),
    gs.grading_date,
    gs.start_time,
    gs.end_time,
    gs.location,
  ...
  WHERE gs.grading_date >= COALESCE(p_from, CURRENT_DATE - INTERVAL '30 days')
    ...
    AND (p_branch_id IS NULL OR COALESCE(s.branch_id, gs.branch_id) = p_branch_id)
