...
serve(async (req) => {
  try {
    ...
    if (roleErr) {
      return new Response(JSON.stringify({ error: "Role check failed", details: roleErr.message }), { status: 500 });
    }
    if (!isSuper) {
      return new Response(JSON.stringify({ error: "Forbidden: superadmin only" }), { status: 403 });
    }
    ...
    if (body.action === "update_email") {
      ...
      if (!oldEmail || !newEmail) {
        return new Response(JSON.stringify({ error: "oldEmail and newEmail are required" }), { status: 400 });
      }
      ...
      if (!user) {
        return new Response(JSON.stringify({ error: "Auth user not found for old email" }), { status: 404 });
      }
